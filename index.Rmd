...
### **NEW:** **Classifying** songs from experimental and typical albums: a closer look at Ummagumma and The Dark Side Of The Moon.
```{r, echo=FALSE}
# umma <- get_playlist_audio_features("spotify", "24ZUwHygqLUwETbG2l0HXs")
# dark <- get_playlist_audio_features("spotify", "67bpTVQpOrtMOwbH6IIhtf")
# ummadark <-
#   bind_rows(
#     umma %>% mutate(playlist = "Ummagumma") %>% slice_head(n = 20),
#     dark %>% mutate(playlist = "The Dark Side Of The Moon") %>% slice_head(n = 20)
#   )
# rm(umma)
# rm(dark)
# 
# indie_features <-
#   ummadark %>%  # For your portfolio, change this to the name of your corpus.
#   add_audio_analysis() %>%
#   mutate(
...
***
To further analyze the difference between experimental albums such as *Ummagumma* and more traditional albums such as *Dark Side*, a classifier was trained on those very albums. The first iteration used all track-level features spotify has to offer: **danceability** and the like, **cepstrum coefficients** and **key** information. A **random forest classifier** was used as it got similar, yet slightly better results than kNN. To ensure accurate representation of the data and to prevent selection bias, a **10-fold cross validation** was maintained. The table below shows that the model has an easier time with *Ummagumma* than with *Dark Side*. This is likely due to *Ummagumma's* songs being a lot more unique. This is what makes it an experimental album, after all. On top of that *Ummagumma* contains 16 songs, compared to *Dark Side's* 10, which means the model has more data points to learn the characteristics of an *Ummagumma* song from.

| Class | Precision | Recall |
|:------:|:-----|:------|
| The Dark Side Of The Moon | 0.778 | 0.700 |
| Ummagumma | 0.824 | 0.875 |
###### Table 1. Performance of random forest model using all features.

In an effort to improve the performance of the model the **10 most important features** were kept, while the rest were discarded. These features are shown in the left-most graph on this page. All other model parameters were kept the same. The table below clearly shows the **tradeoff** that is inherent to the precision and recall measures. As one increases recall, precision decreases and vice versa. This means the overall performance of the model has barely improved, if at all. The table below shows the performance of the revised model, which is graphically represented in the right-most graph on this page.

| Class | Precision | Recall |
|:------:|:-----|:------|
| The Dark Side Of The Moon | 0.727 | 0.800 |
| Ummagumma | 0.867 | 0.813 |
###### Table 2. Performance of random forest model using 10 most imporant features.



<!-- Week 10: Tempograms -->
### Comparing **tempo** in typical and atypical Pink Floyd tracks.
...
floyd_keys <-
  ...
  slice(31:78) %>%  # Limit to 30 tracks
  add_audio_analysis()
